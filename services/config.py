...
class Config:
    ...
    def __init__(self, config_path: str = None):
        # If no config path is provided, look for config.json in the apps/download_file directory
        if config_path is None:
            # Get the absolute path to the config file
            current_file = Path(__file__).resolve()
            config_path = current_file.parent.parent / "apps" / "download_file" / "config.json"
